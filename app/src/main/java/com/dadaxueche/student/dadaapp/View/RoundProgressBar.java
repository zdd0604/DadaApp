...
public class RoundProgressBar extends View {
    ...
    public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        textSize = mTypedArray.getDimension(R.styleable.RoundProgressBar_textSize, 22);
        roundWidth = mTypedArray.getDimension(R.styleable.RoundProgressBar_roundWidth,15);
        max = mTypedArray.getInteger(R.styleable.RoundProgressBar_mymax, 100);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float textWidth = paint.measureText("正确率：" + percent + "%");
        float textWidthf = paint.measureText("未做答");


        if (textIsDisplayable && percent != 0 && style == STROKE) {
            canvas.drawText("正确率：" + percent + "%", centre - textWidth / 2,
                    centre + textSize / 2, paint); //画出进度百分比
        }else{
            canvas.drawText("未做答" , centre - textWidthf / 2,
        ...
        RectF oval = new RectF((float)(centre - radius), centre - radius,
